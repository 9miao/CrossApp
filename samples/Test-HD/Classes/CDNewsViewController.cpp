...
void CDNewsTableCell::initWithCell()
{
    ...
    CAImageView* bg = CAImageView::createWithFrame(CADipRect(0,0,_size.width,_size.height));
    bg->setImage(CAImage::create("image/newscellbg.png"));
    this->getContentView()->addSubview(bg);
    
    theTitle = CALabel::createWithFrame(CADipRect(240, 20, _size.width-250, 40));
    ...
    theDesc = CALabel::createWithFrame(CADipRect(240, 80, _size.width-260, 120));
...
//    p_TableView->setHeaderRefreshView(refreshDiscount1);
    ...
    //initNewsPageView();
    p_Conllection = CAAutoCollectionView::createWithFrame(this->getView()->getBounds());
...
//    RootWindow::getInstance()->getRootNavigationController1()->replaceViewController(viewContrller, false);
...
CACollectionViewCell* CDNewsViewController::collectionCellAtIndex(CACollectionView *collectionView, const CCSize& cellSize, unsigned int section, unsigned int item)
{
    CADipSize _size = cellSize;
    ...
    if (p_Cell == NULL)
    {
        ...
        CommonUrlImageView* theImage = CommonUrlImageView::createWithFrame(CADipRect(20, 20, _size.width-40, _size.height/3*2));
        theImage->setTag(101);
        theImage->setImageViewScaleType(CAImageViewScaleTypeFitImageCrop);
        theImage->setImage(CAImage::create("image/HelloWorld.png"));
        ...
        CALabel* itemText = CALabel::createWithFrame(CADipRect(10, _size.height/3*2+40, itemSize.width-20,80));
        itemText->setTag(100);
...
//    theImage->setUrl(m_msg[row * 3 + item].m_imageUrl[0]);
//    
//    int index = row * 3 + item;
//    
//    CALabel* itemText = (CALabel*)p_Cell->getSubviewByTag(100);
//    itemText->setText(m_msg[row * 3 + item].m_title);
