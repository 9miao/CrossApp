...
m_cbRecvBuf(1024*20),m_cbSendBuf(1024*20),
m_pHandler(netImpl)
{
	m_isvalidSeq = false;
...
{
    printf("SocketClient::~SocketClient");
	m_iState = SocketClient_DESTROY;
	if( m_hSocket!=-1)
		close(m_hSocket);
...
Message* SocketClient::constructMessage(std::string data,int commandId)
{
    Message* msg = new Message();
...
//    str.append(msg->HEAD0);
   ...
   // printf("%s",data);
    memcpy(msg->data+17,data.c_str(),data.size());
...
//			printf("[SocketClient::start()][connect failed][error:%s] \n",error.c_str());
	...
	ByteBuffer * sendBuff=new ByteBuffer(1024);
	 sendBuff = &This->m_cbSendBuf;
...
//            sendBuff.put(&msg, 0, msg->datalength());
			sendBuff->flip();
            printf("%d",socket);
            printf("%s   %d",(char *)sendBuff->getBuffer(),sendBuff->getLimit());
			int ret = send(socket,(char *)sendBuff->getBuffer(),sendBuff->getLimit(),0);
			if(ret == -1){
				This->m_iState = SocketClient_DESTROY;
				string err("发送数据，网络异常！");
...
//					if(DEBUG)printf("server closed connection, receive thread stop %p!\n",This);
				...
				else 
				{
					...
					while(recvBuff->remaining() > tmpOffset){
                        ...
                        {
							//printf("beforece receive message \n");
							Message* message = new Message();				
//							message->type = recvBuff->getInt();
//							message->seqNum = recvBuff->getInt();
                            ...
                            MySocket::shareMySocket()->callBackMessage(message, bytesToInt(message->commandId));
                            //callBack
                            delete message;
						}
                        ...
                        {
							This->m_iState = SocketClient_DESTROY;
//							if(DEBUG)printf("receive error message size(%d) greate capacity(%d), receive thread stop!\n",length,recvBuff->getCapacity());
							MyLock lock(&This->m_sendqueue_mutex);
