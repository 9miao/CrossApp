...
ccArray* ccArrayNew(unsigned int capacity) 
{
	...
	arr->num = 0;
	arr->arr =  (CAObject**)calloc(capacity, sizeof(CAObject*));
	arr->max = capacity;
...
{
	arr->max *= 2;
	CAObject** newArr = (CAObject**)realloc( arr->arr, arr->max * sizeof(CAObject*) );
...
void ccArrayShrink(ccArray *arr)
{
	...
	if (arr->max > arr->num && !(arr->num==0 && arr->max==1))
	{
		...
		arr->arr = (CAObject**)realloc(arr->arr,newSize * sizeof(CAObject*) );
...
unsigned int ccArrayGetIndexOfObject(ccArray *arr, CAObject* object)
{
    const unsigned int arrNum = arr->num;
    CAObject** ptr = arr->arr;
...
bool ccArrayContainsObject(ccArray *arr, CAObject* object)
{
	return ccArrayGetIndexOfObject(arr, object) != CC_INVALID_INDEX;
}

/** Appends an object. Behavior undefined if array doesn't have enough capacity. */
void ccArrayAppendObject(ccArray *arr, CAObject* object)
{
...
void ccArrayAppendObjectWithResize(ccArray *arr, CAObject* object)
{
...
void ccArrayInsertObjectAtIndex(ccArray *arr, CAObject* object, unsigned int index)
{
	...
	if( remaining > 0)
    {
		memmove((void *)&arr->arr[index+1], (void *)&arr->arr[index], sizeof(CAObject*) * remaining );
    }
...
void ccArraySwapObjectsAtIndexes(ccArray *arr, unsigned int index1, unsigned int index2)
{
	...
	CAObject* object1 = arr->arr[index1];
...
void ccArrayRemoveObjectAtIndex(ccArray *arr, unsigned int index, bool bReleaseObj/* = true*/)
{
	...
	if(remaining>0)
    {
		memmove((void *)&arr->arr[index], (void *)&arr->arr[index+1], remaining * sizeof(CAObject*));
    }
}
...
void ccArrayFastRemoveObject(ccArray *arr, CAObject* object)
{
...
void ccArrayRemoveObject(ccArray *arr, CAObject* object, bool bReleaseObj/* = true*/)
{
	unsigned int index = ccArrayGetIndexOfObject(arr, object);
	if (index != CC_INVALID_INDEX)
    {
