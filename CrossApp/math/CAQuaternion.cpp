...
Quaternion::Quaternion(const DPoint3D& axis, float angle)
{
    set(axis, angle);
}
...
void Quaternion::createFromAxisAngle(const DPoint3D& axis, float angle, Quaternion* dst)
{
    GP_ASSERT(dst);
...
void Quaternion::set(const DPoint3D& axis, float angle)
{
...
float Quaternion::toAxisAngle(DPoint3D* axis) const
{
    GP_ASSERT(axis);
