                                     ...
                                     float zNearPlane, float zFarPlane, Mat4* dst)
{
    ...
    {
        CCLOG("Invalid field of view value (%f) causes attempted calculation tan(%f), which is undefined.", fieldOfView, theta);
        return;
    }
...
bool Mat4::decompose(DPoint3D* scale, Quaternion* rotation, DPoint3D* translation) const
{
    ...
    DPoint3D xaxis(m[0], m[1], m[2]);
    float scaleX = xaxis.getLength();

    DPoint3D yaxis(m[4], m[5], m[6]);
    float scaleY = yaxis.getLength();

    DPoint3D zaxis(m[8], m[9], m[10]);
