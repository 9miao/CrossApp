...
bool CAImage::initWithData(const void *data, CAImagePixelFormat pixelFormat, unsigned int pixelsWide, unsigned int pixelsHigh, const CCSize& contentSize)
{
	...
	{
		m_pData = (unsigned char*)malloc(m_nDataLenght);
		
		for (int j = 0; j < pixelsHigh; j++)
		for (int i = 0; i < pixelsWide; i++)
		{
			m_pData[j * pixelsWide + i] = pData[j * pixelsWide + i];
		}
	}
	else
	{
...
CAImage* CAImage::copy()
{
	...
	else if (copyImage->m_ePixelFormat == kCAImagePixelFormat_A8)
	{
		copyImage->m_pData = (unsigned char*)malloc(copyImage->m_nDataLenght);

		for (int j = 0; j < this->m_uPixelsHigh; j++)
		for (int i = 0; i < this->m_uPixelsWide; i++)
		{
			copyImage->m_pData[j * this->m_uPixelsWide + i] = this->m_pData[j * this->m_uPixelsWide + i];
		}
	}
    else
    {
