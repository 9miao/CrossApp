...
#  * REMOTE_CrossApp_REPOSITORY
#  * Note : Ensure you have commit access to above repositories
#  * CrossApp_PULL_BASE
#  * hub
#     * see http://defunkt.io/hub/
#  * Ensure that hub has an OAuth token to REMOTE_CrossApp_REPOSITORY
...
CROSSAPP_ROOT="$CROSSAPP_ROOT" /bin/bash ../../../tojs/genbindings.sh
...
if [ -z "${REMOTE_AUTOGEN_BINDINGS_REPOSITORY+aaa}" ]; then
    ...
    echo to this repo
    echo example
    echo  REMOTE_AUTOGEN_BINDINGS_REPOSITORY=\"git@github.com:folecr/CrossApp-autogen-bindings.git\"
    echo  REMOTE_AUTOGEN_BINDINGS_REPOSITORY=\"\$HOME/test/CrossApp-autogen-bindings\"
    echo
...
if [ -z "${REMOTE_CrossApp_REPOSITORY+aaa}" ]; then
    echo
    echo Environment variable is not set REMOTE_CrossApp_REPOSITORY
    echo This script will NOT automatically push changes
    echo unless this variable is set.
    echo example
    echo  REMOTE_CrossApp_REPOSITORY=\"git@github.com:cocos2d/cocos2d-x.git\"
    echo  REMOTE_CrossApp_REPOSITORY=\"\$HOME/test/cocos2d-x\"
    echo
    echo Exiting with success.
    echo
    exit 0
...
git push "$REMOTE_CrossApp_REPOSITORY" "$COCOS_BRANCH"

if [ -z "${CrossApp_PULL_BASE+aaa}" ]; then
    echo
    echo Environment variable is not set CrossApp_PULL_BASE
    echo This script will NOT automatically generate pull requests
    echo unless this variable is set.
    echo example
    echo  CrossApp_PULL_BASE=\"cocos2d/cocos2d-x:master\"
    echo  CrossApp_PULL_BASE=\"username/repository:branch\"
    echo
    echo Exiting with success.
    echo
    exit 0
...
# 7. 
${HUB} pull-request "$COMMITTAG : updating submodule reference to latest autogenerated bindings" -b "$CrossApp_PULL_BASE" -h "$COCOS_BRANCH"
